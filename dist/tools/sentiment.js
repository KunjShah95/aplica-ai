...
export class SentimentTool {
        ...
        try {
            const response = await axios.post('https://api.openai.com/v1/chat/completions', {
                ...
                messages: [
                    {
                        role: 'system',
                        content: 'You are a sentiment analysis tool. Analyze the sentiment of the following text and return a JSON object with: sentiment (positive/neutral/negative), score (0-1), and emotions (joy, sadness, anger, fear, surprise each 0-1).',
                    },
                    {
                        role: 'user',
                        content: text,
    ...
    async analyzeWithGoogle(text) {
        ...
        try {
            const response = await axios.post(`https://language.googleapis.com/v1/documents:analyzeSentiment?key=${this.googleNLPApiKey}`, {
                document: {
                    type: 'PLAIN_TEXT',
                    content: text,
                },
    ...
    async analyzeEntities(text) {
        ...
        try {
            const response = await axios.post(`https://language.googleapis.com/v1/documents:analyzeEntities?key=${this.googleNLPApiKey}`, {
                document: {
                    type: 'PLAIN_TEXT',
                    content: text,
                },
    ...
    async classifyContent(text) {
        ...
        try {
            const response = await axios.post(`https://language.googleapis.com/v1/documents:classifyText?key=${this.googleNLPApiKey}`, {
                document: {
                    type: 'PLAIN_TEXT',
                    content: text,
                },
