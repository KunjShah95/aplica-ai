...
class RebrandlyService {
    ...
    async shorten(url) {
        ...
        try {
            const response = await axios.post('https://api.rebrandly.com/v1/links', { destination: url }, {
                headers: {
                    'Authorization': `Bearer ${this.apiKey}`,
                    'Content-Type': 'application/json',
                },
...
export class URLShortener {
    ...
    async shorten(url, preferredService) {
        if (preferredService) {
            const service = this.services.find((s) => s.name.toLowerCase() === preferredService.toLowerCase());
            if (service) {
