export class RAGEvaluator {
    ...
    async evaluateRetrieval(query, expectedDocIds, retrievedDocs) {
    ...
    calculateMRR(expectedIds, retrievedIds) {
    ...
    calculateNDCG(expectedIds, retrievedDocs) {
        let dcg = 0;
    ...
    async evaluateGeneration(query, response, context, expectedAnswer) {
    ...
    async calculateFaithfulness(response, context) {
    ...
    isSentenceSupported(sentence, context) {
    ...
    async calculateAnswerRelevance(query, response) {
        const queryKeywords = new Set(query
            .toLowerCase()
            .split(/\s+/)
            .filter((w) => w.length > 3));
        const responseKeywords = new Set(response
            .toLowerCase()
            .split(/\s+/)
            .filter((w) => w.length > 3));
        let matches = 0;
    ...
    calculateContextPrecision(response, context) {
    ...
    async detectHallucinations(response, context, query) {
    ...
    async fullEvaluation(query, expectedDocIds, retrievedDocs, response, context, expectedAnswer) {
        ...
        const retrievalScore = retrieval.scores.precision * 0.3 +
            retrieval.scores.recall * 0.3 +
            retrieval.scores.ndcg * 0.2 +
            retrieval.scores.mrr * 0.2;
        const generationScore = generation.scores.faithfulness * 0.4 +
        ...
        const highSeverityHallucinations = generation.hallucinations.filter((h) => h.severity === 'high');
        const hasMajorIssues = retrievalScore < 0.5 || generationScore < 0.5 || highSeverityHallucinations.length > 0;
        let qualityGrade;
    ...
    setThresholds(config) {
        ...
        if (config.relevanceThreshold)
            this.config.relevanceThreshold = config.relevanceThreshold;
        if (config.retrievalK)
            this.config.retrievalK = config.retrievalK;
    }
}
export class RAGBenchmark {
    ...
    addBenchmarkCase(query, expectedDocIds, expectedAnswer) {
        this.benchmarkQueries.push({ query, expectedDocIds, expectedAnswer });
    }
    async runBenchmark(retrievalFn, generationFn) {
        const results = [];
        for (const benchmarkCase of this.benchmarkQueries) {
            ...
            const result = await this.evaluator.fullEvaluation(benchmarkCase.query, benchmarkCase.expectedDocIds, retrievedDocs, response, context, benchmarkCase.expectedAnswer);
            results.push(result);
        }
        const avgPrecision = results.reduce((sum, r) => sum + r.retrieval.scores.precision, 0) / results.length;
        const avgRecall = results.reduce((sum, r) => sum + r.retrieval.scores.recall, 0) / results.length;
        const avgNDCG = results.reduce((sum, r) => sum + r.retrieval.scores.ndcg, 0) / results.length;
        const avgFaithfulness = results.reduce((sum, r) => sum + r.generation.scores.faithfulness, 0) / results.length;
        const avgRelevance = results.reduce((sum, r) => sum + r.generation.scores.answerRelevance, 0) / results.length;
        const hallucinationCount = results.reduce((sum, r) => sum + r.generation.hallucinations.length, 0);
        return {
