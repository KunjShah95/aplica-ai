...
export class SLATracker extends EventEmitter {
    ...
    recordRequest(durationMs, success) {
        const now = new Date();
        let currentMetric = this.metrics.find((m) => m.timestamp.getTime() === this.getHourBucket(now).getTime());
        if (!currentMetric) {
            currentMetric = {
    ...
    checkAlerts(durationMs, success) {
    ...
    updateIncidentStatus(incidentId, status, message, author = 'System') {
    ...
    getUptimeReport() {
        ...
        const recentIncidents = this.incidents.filter((i) => i.startedAt >= weekAgo && i.status !== 'resolved');
        ...
        const mttr = resolvedIncidents.length > 0 ? totalDowntime / resolvedIncidents.length / 1000 / 60 : 0;
        return {
    ...
    setAlertThresholds(thresholds) {
        if (thresholds.responseTime)
            this.alertThresholds.responseTime = thresholds.responseTime;
        if (thresholds.errorRate)
            this.alertThresholds.errorRate = thresholds.errorRate;
        if (thresholds.uptime)
            this.alertThresholds.uptime = thresholds.uptime;
    }
    getSLAStatus() {
        const current = this.getCurrentStatus();
        const meetingTarget = current.uptime >= this.config.uptimeTarget * 100 &&
