...
export class CriticAgent extends Agent {
    reviewHistory = [];
    approvalThreshold = 7; // Minimum score to pass review
    constructor(options) {
        super(options);
    }
    /**
    ...
    checkClarity(draft) {
        ...
        const complexWordCount = complexWords.filter((w) => new RegExp(`\\b${w}\\b`, 'i').test(draft)).length;
        return complexWordCount > 3;
    }
    /**
     * Check for tone issues
     */
    checkTone(draft, audience) {
        if (!audience)
            return false;
     ...
     * Generate feedback
     */
    generateFeedback(scores, context) {
        const comments = [];
        if (scores.clarity < 7) {
     ...
     */
    getSuggestedImprovements(scores) {
        const improvements = [];
        if (scores.clarity < 8) {
    ...
    getStats() {
        ...
        return {
            ...
            avgScore: this.reviewHistory.reduce((a, r) => a + r.critique.scores.overall, 0) /
 ...
 */
export function createCriticTeamAgent(options) {
