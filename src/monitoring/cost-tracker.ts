...
export interface UserCostSummary {
  ...
  totalTokens: number;
  sessionCount: number;
  dailyBreakdown: { date: string; costUsd: number; tokens: number }[];
  modelBreakdown: { model: string; costUsd: number; tokens: number }[];
}

const MODEL_PRICING: TokenPricing[] = [
...
export class CostTracker {
  ...
  getUserCostSummary(userId: string, sinceDays = 30): UserCostSummary {
  ...
  getGlobalStats(): {
    ...
    totalCostUsd: number;
    totalTokens: number;
    byProvider: { provider: string; costUsd: number; tokens: number }[];
  } {
    const providerMap = new Map<string, { costUsd: number; tokens: number }>();
  ...
  getSupportedModels(): { provider: string; model: string }[] {
