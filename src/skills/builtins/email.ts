...
export class EmailSkill implements Skill {
  manifest = manifest;
  private mockEmails: Map<
    string,
    Array<{ id: string; from: string; to: string; subject: string; body: string; date: Date }>
  > = new Map();
  ...
  private async draftEmail(params: Record<string, unknown>): Promise<SkillResult> {
    const to = params.to as string;
    const subject = params.subject as string;
    const body = params.body as string;
  ...
  private async readEmails(params: Record<string, unknown>, userId: string): Promise<SkillResult> {
    ...
    const formatted = emails
      .map(
        (e) =>
          `[${e.date.toISOString()}] From: ${e.from}\nSubject: ${e.subject}\n${e.body.slice(0, 100)}...`
      )
      .join('\n---\n');
  ...
  private async listEmails(params: Record<string, unknown>, userId: string): Promise<SkillResult> {
  ...
  private async searchEmails(
    ...
    const emails = this.getEmails(userId).filter(
      (e) =>
  ...
  private getEmails(
    userId: string
  ): typeof this.mockEmails extends Map<string, infer V> ? V : never[] {
