...
export class CodeSkill implements Skill {
  ...
  async execute(context: SkillExecutionContext): Promise<SkillResult> {
    ...
    try {
      switch (action) {
        case 'write':
          const writePath = parameters.path as string;
        ...
        case 'read':
          const readPath = parameters.path as string;
          if (!readPath) {
        ...
        case 'run':
          const code = parameters.content as string;
          const language = (parameters.language as 'javascript' | 'typescript') || 'javascript';
          if (!code) {
        ...
        case 'list':
          const listPath = (parameters.path as string) || '.';
        ...
        case 'delete':
          const deletePath = parameters.path as string;
          if (!deletePath) {
            return { success: false, output: 'Path is required' };
          }
  ...
  private formatFileList(data: string | undefined): string {
    if (!data) return '';
    try {
      const files = JSON.parse(data) as Array<{ name: string; isDirectory: boolean; size: number }>;
      return files
        .map(
          (f) =>
  ...
  private countResults(data: string | undefined): number {
    if (!data) return 0;
    try {
