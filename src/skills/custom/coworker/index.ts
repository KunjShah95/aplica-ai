...
import { LLMMessage } from '../../../core/llm/base.js';
...
export class CoworkerSkill implements Skill {
    manifest: any;
    ...
    async execute(context: SkillExecutionContext): Promise<SkillResult> {
        ...
        if (!task) return { success: false, output: 'Task is required.' };
...
`;
                ...
                try {
                    ...
                    if (toolName === 'run_command') {
                        ...
                        if (this.page) {
                            ...
                            try {
                                if (direction === 'top') await this.page.evaluate(() => window.scrollTo(0, 0));
                                else if (direction === 'bottom') await this.page.evaluate(() => window.scrollTo(0, document.body.scrollHeight));
                                else if (direction === 'up') await this.page.evaluate((amount) => window.scrollBy(0, -amount), amount);
                                else await this.page.evaluate((amount) => window.scrollBy(0, amount), amount);
            ...
            if (sandboxStarted) await sandbox.stop();
        }
    }
}
