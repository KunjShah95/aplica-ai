...
export class TaskQueue {
  ...
  private getLanePriority(lane: string): number {
    const laneConfig = this.lanes.find((l) => l.name === lane);
    return laneConfig ? laneConfig.priority : 2;
  }

  async getJobCounts(lane: string): Promise<Record<TaskStatus, number>> {
    const queue = this.queues.get(lane);
    if (!queue) {
      throw new Error(`Unknown lane: ${lane}`);
    }
  ...
  async getStatus(): Promise<Record<string, Record<TaskStatus, number>>> {
    const status: Record<string, Record<TaskStatus, number>> = {};
  ...
  async process(type: TaskType, handler: (job: Job<TaskPayload>) => Promise<void>): Promise<void> {
    for (const lane of this.lanes) {
  ...
  async pause(lane: string): Promise<void> {
    const queue = this.queues.get(lane);
    if (queue) {
      await queue.pause();
  ...
  async resume(lane: string): Promise<void> {
    const queue = this.queues.get(lane);
    if (queue) {
      await queue.resume();
  ...
  async clean(lane: string, grace: number = 0): Promise<void> {
    const queue = this.queues.get(lane);
    if (queue) {
      await (queue as any).clean(grace, 'completed');
      await (queue as any).clean(grace, 'failed');
