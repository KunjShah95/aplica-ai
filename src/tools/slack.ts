...
export class SlackTool {
  ...
  async sendDirectMessage(userId: string, text: string, blocks?: any[]): Promise<Message | null> {
    try {
      const conversation = await this.client.conversations.open({
        users: userId,
      } as any);

      if (!conversation.channel?.id) return null;
  ...
  async updateMessage(channel: string, ts: string, text: string, blocks?: any[]): Promise<boolean> {
    try {
  ...
  async removeReaction(channel: string, ts: string, emoji: string): Promise<boolean> {
    try {
  ...
  async getMessage(channel: string, ts: string): Promise<Message | null> {
    try {
