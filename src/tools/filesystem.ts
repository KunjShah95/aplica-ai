...
export class FilesystemTool {
  ...
  async readFile(filePath: string, options?: FileOperationOptions): Promise<ReadResult> {
    try {
  ...
  async searchFiles(options: FileSearchOptions): Promise<string[]> {
  ...
  async searchInFiles(
    searchPath: string,
    pattern: string | RegExp,
    options?: { encoding?: BufferEncoding; maxResults?: number }
  ...
  async readJSON<T>(filePath: string): Promise<{ success: boolean; data?: T; error?: string }> {
  ...
  async writeJSON(filePath: string, data: any, pretty = true): Promise<WriteResult> {
    const content = pretty ? JSON.stringify(data, null, 2) : JSON.stringify(data);
    return this.writeFile(filePath, content);
  }

  async getDiskUsage(directory?: string): Promise<{ total: number; used: number; free: number }> {
  ...
  private isAllowedPath(pathToCheck: string): boolean {
  ...
  private matchesType(fileName: string, types?: string[]): boolean {
