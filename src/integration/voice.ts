...
export class VoiceService {
  ...
  async textToSpeech(options: TTSOptions): Promise<Buffer | string> {
    if (!this.openai) {
  ...
  async speechToText(options: STTOptions): Promise<TranscriptionResult> {
    if (!this.openai) {
      throw new Error('OpenAI client not initialized. Set API key first.');
    }
  ...
  async translateAudio(audioBuffer: Buffer): Promise<TranscriptionResult> {
    if (!this.openai) {
      throw new Error('OpenAI client not initialized. Set API key first.');
    }
  ...
  async generateSpeechWithPreset(
    text: string,
    presetName: string,
...
<speak version="1.1" xmlns="http://www.w3.org/2001/10/synthesis" xml:lang="en-US">
  ...
  async textToSpeechSsml(ssml: string, options?: Partial<TTSOptions>): Promise<Buffer | string> {
    if (!this.openai) {
      throw new Error('OpenAI client not initialized. Set API key first.');
    }
  ...
  private parseMp3Header(buffer: Buffer): { duration: number } | null {
    try {
