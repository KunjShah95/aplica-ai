...
export class MarkdownMemory {
  ...
  private dailyLogsEnabled: boolean;
  ...
  async createNote(note: Omit<MarkdownNote, 'createdAt' | 'updatedAt'>): Promise<MarkdownNote> {
    if (!this.notesEnabled) {
      throw new Error('Notes are not enabled');
    }

    const now = new Date();
    const fullNote: MarkdownNote = {
      ...note,
      createdAt: now,
      updatedAt: now,
    };
  ...
  async updateNote(
    fileName: string,
    updates: Partial<Omit<MarkdownNote, 'createdAt'>>
  ...
  async readNote(fileName: string): Promise<MarkdownNote | null> {
    if (!this.notesEnabled) {
      throw new Error('Notes are not enabled');
    }
  ...
  async listNotes(category?: string): Promise<{ fileName: string; note: MarkdownNote }[]> {
    ...
    const notes: { fileName: string; note: MarkdownNote }[] = [];
  ...
  async addLogEntry(entry: Omit<LogEntry, 'time'>): Promise<DailyLog> {
  ...
  async searchNotes(
    ...
    const results: { fileName: string; note: MarkdownNote; score: number }[] = [];
  ...
  private renderNoteToMarkdown(note: MarkdownNote): string {
...
`;
  ...
  private parseMarkdownToNote(content: string): MarkdownNote | null {
  ...
  private renderLogToMarkdown(log: DailyLog): string {
    return `---
date: ${log.date}
...
`;
  ...
  private async parseMarkdownToLog(content: string): Promise<DailyLog> {
    const frontmatterMatch = content.match(/^---\n([\s\S]*?)\n---\n([\s\S]*)$/);
    if (!frontmatterMatch) {
      return { date: '', entries: [] };
    }
