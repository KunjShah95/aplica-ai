...
interface PatternExtractor {
  name: string;
  extract: (messages: DailyEpisode['messages']) => Array<{ description: string; confidence: number; evidence: string }>;
}

export class EpisodicReplay {
  private patternExtractors: PatternExtractor[] = [];
  ...
  async runDailyReplay(userId: string, date: Date): Promise<EpisodeInsight[]> {
    ...
    for (const extractor of this.patternExtractors) {
      ...
      for (const item of extracted) {
        ...
        if (existingInsight) {
          ...
          const newInsight: EpisodeInsight = {
            id: randomUUID(),
            userId,
  ...
  private async findSimilarInsight(userId: string, description: string): Promise<EpisodeInsight | null> {
  ...
  private async updateInsight(insight: EpisodeInsight): Promise<void> {
  ...
  private async storeInsight(insight: EpisodeInsight): Promise<void> {
  ...
  private extractTimePatterns(messages: DailyEpisode['messages']): Array<{ description: string; confidence: number; evidence: string }> {
    const patterns: Array<{ description: string; confidence: number; evidence: string }> = [];
  ...
  private extractTonePatterns(messages: DailyEpisode['messages']): Array<{ description: string; confidence: number; evidence: string }> {
    const patterns: Array<{ description: string; confidence: number; evidence: string }> = [];
    const userMessages = messages.filter(m => m.role === 'USER');
  ...
  private extractTopicPatterns(messages: DailyEpisode['messages']): Array<{ description: string; confidence: number; evidence: string }> {
  ...
  private extractInteractionPatterns(messages: DailyEpisode['messages']): Array<{ description: string; confidence: number; evidence: string }> {
    const patterns: Array<{ description: string; confidence: number; evidence: string }> = [];
    const userMessages = messages.filter(m => m.role === 'USER');
    ...
    const thanksCount = userMessages.filter(m => 
      m.content.toLowerCase().includes('thank') || m.content.toLowerCase().includes('thanks')
    ).length;
  ...
  private extractStressPatterns(messages: DailyEpisode['messages']): Array<{ description: string; confidence: number; evidence: string }> {
    const patterns: Array<{ description: string; confidence: number; evidence: string }> = [];
    const userMessages = messages.filter(m => m.role === 'USER');
    ...
    for (const msg of userMessages) {
      ...
      if (stressKeywords.some(k => lower.includes(k))) stressCount++;
      if (urgencyKeywords.some(k => lower.includes(k))) urgencyCount++;
    }
  ...
  private mapExtractorToType(extractorName: string): EpisodeInsight['type'] {
    const mapping: Record<string, EpisodeInsight['type']> = {
  ...
  async getInsights(userId: string, _types?: EpisodeInsight['type'][]): Promise<EpisodeInsight[]> {
