...
export class AnalystAgent extends Agent {
   ...
   * Parse and validate data
   */
  private parseData(data: any[], columns?: string[]): any[] {
   ...
   */
  private normalizeValue(value: any): number | string | boolean | null {
   ...
   */
  private calculateStatistics(data: any[]): Record<string, ColumnStatistics> {
    ...
    for (const col of columns) {
      ...
      if (isNumeric) {
        ...
        stats[col] = {
          ...
          topValues: this.getTopValues(values as string[]),
        };
      }
    }
   ...
   */
  private getTopValues(values: string[], limit: number = 5): { value: string; count: number }[] {
   ...
   */
  private generateInsights(stats: Record<string, ColumnStatistics>): Insight[] {
  ...
  private async createVisualizations(
    data: any[],
    stats: Record<string, ColumnStatistics>
    ...
    if (numericCols.length > 0) {
      visualizations.push({
        ...
        data: numericCols.slice(0, 3).map((col) => ({
          column: col,
          distribution: (stats[col] as any).distribution,
        })),
      });
    }
    ...
    if (numericCols.length >= 2) {
      visualizations.push({
        ...
        data: {
          ...
          matrix: numericCols.slice(0, 4).map((col1) =>
            numericCols.slice(0, 4).map((col2) => ({
              col1,
              col2,
  ...
  private generateNarrative(
    ...
    sections.push(
      `This analysis covers ${insights.length} key insights across the dataset.`
    );
    sections.push('');
    ...
    sections.push(
      'The analysis includes a distribution histogram and correlation heatmap to visualize the data patterns.'
    );
   ...
   * Generate summary
   */
  private generateSummary(narrative: string, insights: Insight[]): string {
    return insights
      .slice(0, 3)
   ...
   */
  async runPandasStyleAnalysis(
    csvData: string,
    options?: { header?: boolean; separator?: string }
  ): Promise<AnalysisReport> {
    // Parse CSV
  ...
  getStats(): {
    ...
    const totalInsights = this.analysisHistory.reduce(
      (a, r) => a + r.report.insights.length,
      0
    );
 ...
 */
export function createAnalystAgent(options: AgentOptions): AnalystAgent {
