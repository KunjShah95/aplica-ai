...
export class ScratchpadAgent extends Agent {
  ...
  private thinkingThreshold = 5; // Minimum steps before executing
   ...
   */
  async processWithScratchpad(content: string): Promise<{ response: string; reasoning: string[] }> {
   ...
   */
  private generateResponse(isReady: boolean): string {
    if (!isReady) {
  ...
  ): Promise<{ response: string; reasoning: string[]; result?: unknown; error?: string }> {
 ...
 */
export function createScratchpadAgent(options: AgentOptions): ScratchpadAgent {
